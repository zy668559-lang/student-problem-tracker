  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="本周上传次数" value={snapshot.weeklyUploadCount} detail="家长或学生本周已上传的资料次数。" />
        <MetricCard label="主要问题数" value={snapshot.weeklyProblems.length || 0} detail="从最近诊断中抽取的高频问题标签。" />
        <MetricCard label="修复动作数" value={snapshot.weeklyActions.length || 0} detail="系统自动生成并入档的修复动作。" />
        <MetricCard label="变化记录" value={snapshot.weeklyChanges.length || 0} detail="最近被记录的改善或回弹现象。" />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <SectionCard title="本周变化情况" subtitle="最近是否真正发生变化。">
          <ul className="space-y-3 text-sm leading-6 text-slate">
            ...
            )) : <li className="text-slate">等待新上传后生成变化记录。</li>}
          </ul>
        </SectionCard>
      ...
      <SectionCard title="快速入口" subtitle="直接进入上传、诊断和周总结页面。">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Link
            href={snapshot.latestDiagnosisId ? `/diagnosis/${snapshot.latestDiagnosisId}` : "/upload"}
            className="rounded-3xl border border-line bg-mist px-5 py-5"
          >
            <p className="text-lg font-semibold text-ink">查看最新诊断</p>
            <p className="mt-2 text-sm text-slate">检查结构化 JSON 和修复动作</p>
          </Link>
          <Link
            href={snapshot.latestWeeklyReportId ? `/weekly-report/${snapshot.latestWeeklyReportId}` : "/review-queue"}
            className="rounded-3xl border border-line bg-white px-5 py-5"
          >
            <p className="text-lg font-semibold text-ink">查看周总结</p>
            <p className="mt-2 text-sm text-slate">聚合一周变化、未稳住项和下周计划</p>
          </Link>
        </div>
      </SectionCard>
    </div>
